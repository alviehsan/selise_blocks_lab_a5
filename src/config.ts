...
export const reportScenarios: ReportScenario[] = [
  ...
  },
  {
    ...
    gdprNotes: "No raw credentials are stored in the report draft; recipient actions require explicit approval.",
    draft: {
  ...
  },
  {
    id: "gdpr-rls",
    name: "GDPR + RLS",
    summary: "Owner-scoped access, data minimisation, retention.",
    complexity: 3,
    blocksServices: ["Data Gateway", "Access Policies", "Observability"],
    acceptanceChecks: [
      "LabNote owner-scoped RLS prevents cross-user reads",
      "GDPR endpoint reports policy, lawful basis, retention, rights",
      "Trace endpoint shows authenticated UDS calls under the right projectKey",
    ],
    workflowPlan: "Workflow fetches gdpr-report.json and scenario-status.json, then invokes an Agent that cites the policy from KB.",
    gdprNotes: "Personal data minimisation, role-scoped access, retention bounded, deletion path verified.",
    draft: {
      title: "Blocks Lab GDPR + RLS Brief",
      audience: "Compliance reviewers",
      source: "gdpr-report.json, owner-scoped LabNote, KB glossary",
      language: "en",
      includeGateway: true,
      includeAgent: false,
      includeWorkflow: true,
